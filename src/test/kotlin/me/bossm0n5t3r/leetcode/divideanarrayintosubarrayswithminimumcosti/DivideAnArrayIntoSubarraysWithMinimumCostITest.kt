...
class DivideAnArrayIntoSubarraysWithMinimumCostITest {
    private val sut = DivideAnArrayIntoSubarraysWithMinimumCostI.Solution()
    ...
    fun test() {
        val testDataList =
            listOf(
                ...
                TestData("[5,4,3]".toIntArray(), 12),
                TestData("[10,3,1,1]".toIntArray(), 12),
            )

        for (testData in testDataList) {
            assertEquals(
                testData.result,
                sut.minimumCost(testData.nums),
            )
        }
    }
}
