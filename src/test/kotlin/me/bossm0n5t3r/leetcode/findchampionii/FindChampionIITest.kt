package me.bossm0n5t3r.leetcode.findchampionii

import me.bossm0n5t3r.leetcode.utils.StringUtil.toArrayOfIntArray
import org.junit.jupiter.api.Assertions.assertEquals
import org.junit.jupiter.api.Test

class FindChampionIITest {
    private val sut = FindChampionII.Solution()

    private data class TestData(
        val n: Int,
        val edges: Array<IntArray>,
        val result: Int,
    ) {
        override fun equals(other: Any?): Boolean {
            if (this === other) return true
            if (javaClass != other?.javaClass) return false

            other as TestData

            if (n != other.n) return false
            if (result != other.result) return false
            if (!edges.contentDeepEquals(other.edges)) return false

            return true
        }

        override fun hashCode(): Int {
            var result1 = n
            result1 = 31 * result1 + result
            result1 = 31 * result1 + edges.contentDeepHashCode()
            return result1
        }
    }

    @Test
    fun test() {
        val testDataList =
            listOf(
                TestData(3, "[[0,1],[1,2]]".toArrayOfIntArray(), 0),
                TestData(4, "[[0,2],[1,3],[1,2]]".toArrayOfIntArray(), -1),
            )

        for (testData in testDataList) {
            assertEquals(
                testData.result,
                sut.findChampion(testData.n, testData.edges),
            )
        }
    }
}
